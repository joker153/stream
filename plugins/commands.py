...
from info import CHANNELS, ADMINS, AUTH_CHANNEL, AUTH_CHANNEL2, LOG_CHANNEL, PICS, BATCH_FILE_CAPTION, CUSTOM_FILE_CAPTION, PROTECT_CONTENT, MSG_ALRT, MAIN_CHANNEL
...
async def start(client, message):
    ...
    if len(message.command) != 2:
        ...
        return
    if (AUTH_CHANNEL and not await is_subscribed(client, message)) or (AUTH_CHANNEL2 and not await is_subscribed_force2(client, message)):
        try:
...
async def deletemultiplefiles(bot, message):
    btn = [[
            ...
            InlineKeyboardButton("Delete Srt", callback_data="srt")  
          ]]
    await message.reply_text(
